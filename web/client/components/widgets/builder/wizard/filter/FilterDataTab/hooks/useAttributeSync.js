...
export const useAttributeSync = (filterData, onChangeProp) => {
    return useCallback((key, value) => {
        ...
        if (key === 'data.valueAttribute' && value) {
            ...
            // Set labelAttribute to the same value only if it's undefined or null
            if (currentData.labelAttribute === null) {
                onChangeProp('data.labelAttribute', value);
            }

            // Set sortByAttribute to the same value only if it's undefined or null
            if (currentData.sortByAttribute === null) {
