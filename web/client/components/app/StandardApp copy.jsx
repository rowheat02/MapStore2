...
class StandardApp extends React.Component {
    ...
    addProjDefinitions(config) {
        if (config.projectionDefs && config.projectionDefs.length) {
            return import('proj4').then((mod) => {
                ...
                console.log("projectionDefs", config.projectionDefs);
                config.projectionDefs.forEach((proj) => {
                    proj4.defs(proj.code, proj.def);
                });
                // Check registered projections
                const registeredProjections = {};
                for (let code in proj4.defs) {
                    if (proj4.defs.hasOwnProperty(code)) {
                        registeredProjections[code] = proj4.defs[code];
                    }
                }
                // eslint-disable-next-line no-console
                console.log("Registered projections:", Object.keys(registeredProjections));
                // eslint-disable-next-line no-console
                console.log("All registered defs:", registeredProjections);

                // Step 1: Load and register grid files first (if they exist)
                const gridPromise = config.gridFiles
                    ? this.registerGridFiles(config.gridFiles, proj4)
                    : Promise.resolve();

                // Step 2: Register projection definitions (always execute after Step 1)
                return gridPromise.then(() => {
                    // Re-register projection definitions after grid files are loaded
                    config.projectionDefs.forEach((proj) => {
                        proj4.defs(proj.code, proj.def);
                    });
                    // eslint-disable-next-line no-console
                    console.log("Projections re-registered after grid files");
                });
            });
        }
    ...
    registerGridFiles(gridFiles, proj4) {
            ...
            return new Promise((resolve) => {
                if (gridInfo.type === 'gsb') {
                    ...
                    fetch(gridInfo.path)
                        ...
                        .then(gridData => {
                            proj4.nadgrid(gridName, gridData);
                            // eslint-disable-next-line no-console
                            console.log(`Grid ${gridName} registered successfully`);
                            resolve();
                        })
                        .catch(error => {
                            // eslint-disable-next-line no-console
                    ...
                    import('geotiff').then(({ fromUrl }) => {
                        ...
                        return proj4.nadgrid(gridName, tiff).ready;
                    }).then(() => {
                        // eslint-disable-next-line no-console
                        console.log(`Grid ${gridName} registered successfully`);
                        resolve();
