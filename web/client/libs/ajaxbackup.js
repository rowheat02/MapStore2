...
const { shouldUseProxy, setCORSDisabledUrl } = require('../api/CORS');
...
axios.interceptors.request.use(config => {
    ...
    if (!sameOrigin) {
        ...
        if (proxyUrl) {
            ...
            const cannotUseCORS = shouldUseProxy(uri);
...
axios.interceptors.response.use(response => response, (error) => {
    if (error.config && error.config.autoDetectCORS) {
        ...
        if (shouldUseProxy(error.config.url) && typeof error.response === 'undefined') {
            // check
            setCORSDisabledUrl(error.config.url);
