...
function updateCheckedStatus(data, updatedId, newChecked) {
    ...
    function updateParents(node, parent) {
        ...
        const childStates = parent.children.map(child => ({
    ...
    function processNode(node, parent) {
        // Attach reference to parent for upward flow
        node.parent = parent;
...
const TreeNode = ({ node, onToggle, onCheck }) => {
    ...
    // since indeterminate is not React state property so handled through ref
    useEffect(()=>{
    ...
    return (
        <div style={{ marginLeft: 20 }}>
            <div>
                ...
                <input ref={inputRef} type="checkbox" style={{marginRight: 3}} checked={node.checked || false} onChange={handleCheck} />
                {/* {node.label} */}
...
const Tree = ({ data, onTreeUpdate }) => {
    ...
    const handleToggle = (node) => {
        ...
        // console.log(updateTree(treeData), 'updateTree(treeData)');
    };
    ...
    const handleCheck = (node, isChecked) => {
        ...
        // updateCheckedStatus is used to updated children's and parent's checked, indeterminate state if needed
