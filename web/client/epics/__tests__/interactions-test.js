...
import { UPDATE_PROPERTY, DELETE, UPDATE } from '../../actions/widgets';
...
describe('interactions epics', () => {
    ...
    describe('cleanupAndReapplyFilterWidgetInteractionsEpic', () => {
        ...
        it('on UPDATE filter widget, resets connected chart axis applied current time before reapplying it', (done) => {
            const filterWidget = makeFilterWidget({
                selections: { [FILTER_ID]: ['2021-01-01T00:00:00Z'] },
                interactions: [{
                    id: 'int-chart-axis-time',
                    plugged: true,
                    targetType: 'applyDimension',
            ...
            testEpic(
                cleanupAndReapplyFilterWidgetInteractionsEpic,
                2,
                [{ type: UPDATE, widget: filterWidget, target: 'floating' }],
                (actions) => {
                    expect(actions[0].type).toBe(UPDATE_PROPERTY);
                    expect(actions[0].id).toBe(CHART_WIDGET_ID);
                    expect(actions[0].key).toBe('charts[0].xAxisOpts[1].appliedCurrentTime');
                    expect(actions[0].value).toBe(null);
                    expect(actions[1].type).toBe(UPDATE_PROPERTY);
                    expect(actions[1].id).toBe(CHART_WIDGET_ID);
                    expect(actions[1].key).toBe('charts[0].xAxisOpts[1].appliedCurrentTime');
                    expect(actions[1].value).toBe('2021-01-01T00:00:00Z');
                },
                state,
                done
            );
