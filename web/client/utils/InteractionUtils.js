...
function createChartAxisCurrentTimeNode(axis, axisKey, chartId, traceId) {
    return {
        type: 'element',
...
function createChartAxisNode(axis, axisKey, chartId, traceId) {
    ...
    return {
        ...
        nodePathMode: 'dot',
        preserveWhenSingleChild: true,
        children: [createChartAxisCurrentTimeNode(axis, axisKey, chartId, traceId)]
    };
}
...
function getTraceDateAxisNodes(trace = {}, chart = {}, axisKey) {
    ...
    return axisOpts
        ...
        .map(({ axis }) => createChartAxisNode(axis, axisKey, chart?.chartId || chart?.id, trace?.id));
}
