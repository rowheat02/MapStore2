 ...
 * @param {object} initialMapConfig - Initial map configuration from mapConfigRawData
...
export const compareMapDataChanges = (currentMapData, initialMapConfig) => {
    ...
    console.log(currentMapData, initialMapConfig, 'currentMapData, initialMapConfig');

    return !MapUtils.compareMapChanges(initialMapConfig, currentMapData);
};

export const computeSaveResource = (initialResource, resource, resourceData, resourceType) => {
    const {
        pendingChanges,
        metadata,
        linkedResources,
    ...
    if (resourceType === 'MAP' && resourceData?.payload) {
    // If payload contains raw map data (not composed), compose it now
 ...
 * @param {string} resourceData.resourceType - The type of resource (e.g., 'MAP', 'DASHBOARD')
...
const computeDataChanges = (resourceData) => {
    ...
    if (resourceType === 'MAP') {
        // Use proper comparison logic for map resources
